...
const _isDate = value => isDate_default()(value) && !isNaN(value.getTime());
const isObject = value => getType(value) === 'Object'; // Object utils

const _has = has_default.a;
const hasAny = (obj, props) => some_default()(props, p => has_default()(obj, p)); // Collection utils
...
const evalFn = (fn, args) => isFunction_default()(fn) ? fn(args) : fn;
...
const arrayHasItems = array => isArrayLikeObject_default()(array) && array.length;
...
toPairs_default()(locales_locales).forEach(([id, {
  dow,
  L
}]) => {
  locales_locales[id] = {
...
const setupDefaults = opts => {
  if (!defaults_) {
    defaults_ = new external_commonjs_vue_commonjs2_vue_root_Vue_default.a({
      ...
      computed: {
        locales() {
          return mapValues_default()(this.defaults.locales, v => {
            v.masks = defaultsDeep_default()(v.masks, this.defaults.masks);
            return v;
          });
        }
...
class theme_Theme {
  ...
  normalizeAttr({
    ...
    toPairs_default()(root).forEach(([targetType, targetConfig]) => {
  ...
  normalizeHighlight(config) {
    ...
    toPairs_default()(highlight).forEach(([targetConfig]) => {
  ...
  normalizeNonHighlight(type, config, styleFn) {
    ...
    toPairs_default()(attr).forEach(([targetConfig]) => {
...
class locale_Locale {
  ...
  normalizeMasks(masks) {
    return (arrayHasItems(masks) && masks || [isString_default()(masks) && masks || 'YYYY-MM-DD']).map(m => maskMacros.reduce((prev, curr) => prev.replace(curr, this.masks[curr] || ''), m));
  }
...
function buildMediaQuery(screens) {
  ...
  return screens.map(screen => {
    ...
    return map_default()(screen, (value, feature) => {
      feature = get_default()({
        min: 'min-width',
        max: 'max-width'
      }, feature, feature);
  ...
  screensComp = new external_commonjs_vue_commonjs2_vue_root_Vue_default.a({
    ...
    methods: {
      refreshQueries() {
        ...
        this.queries = mapValues_default()(screens, v => {
      ...
      refreshMatches() {
        this.matches = toPairs_default()(this.queries).filter(p => p[1].matches).map(p => p[0]);
      }
...
external_commonjs_vue_commonjs2_vue_root_Vue_default.a.mixin({
  ...
  computed: {
    $screens() {
      return (config, def) => screensComp.matches.reduce((prev, curr) => _has(config, curr) ? config[curr] : prev, isUndefined_default()(def) ? config.default : def);
    }
...
class dateInfo_DateInfo {
  constructor(config, {
    ...
    if (this.isRange) {
      ...
      if (config.on) {
        const or = (isArrayLikeObject_default()(config.on) ? config.on : [config.on]).map(o => {
  ...
  static get patterns() {
    return {
      ...
      days: {
        validate: days => isArrayLikeObject_default()(days) ? days : [parseInt(days, 10)],
        test: (day, days) => days.includes(day.day) || days.includes(-day.dayFromEnd)
      },
      weekdays: {
        validate: weekdays => isArrayLikeObject_default()(weekdays) ? weekdays : [parseInt(weekdays, 10)],
      ...
      weeks: {
        validate: weeks => isArrayLikeObject_default()(weeks) ? weeks : [parseInt(weeks, 10)],
        test: (day, weeks) => weeks.includes(day.week) || weeks.includes(-day.weekFromEnd)
      },
      months: {
        validate: months => isArrayLikeObject_default()(months) ? months : [parseInt(months, 10)],
        test: (day, months) => months.includes(day.month)
      },
      years: {
        validate: years => isArrayLikeObject_default()(years) ? years : [parseInt(years, 10)],
  ...
  static get patternProps() {
    return Object.keys(dateInfo_DateInfo.patterns).map(k => ({
      name: k,
  ...
  static testConfig(config, day, dateInfo) {
    ...
    if (isObject(config)) {
      return Object.keys(config).every(k => dateInfo_DateInfo.patterns[k].test(day, config[k], dateInfo));
    }
  ...
  matchesDay(day) {
    ...
    if (this.on.or && !this.on.or.some(or => dateInfo_DateInfo.testConfig(or, day, this))) {
      return false;
    } // Patterns match
...
class attribute_Attribute {
  constructor({
    ...
    this.dates = (this.hasDates && this.dates || this.hasExcludeDates && [{}] || []).map(d => d && (d instanceof dateInfo_DateInfo ? d : new dateInfo_DateInfo(d, this.dateOpts))).filter(d => d); // Assign final exclude dates

    this.excludeDates = (this.hasExcludeDates && this.excludeDates || []).map(d => d && (d instanceof dateInfo_DateInfo ? d : new dateInfo_DateInfo(d, this.dateOpts))).filter(d => d);
    this.isComplex = _some(this.dates, d => d.isComplex);
...
/* harmony default export */ var CalendarNavvue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    firstYear() {
      return head_default()(this.yearItems.map(i => i.year));
    },

    lastYear() {
      return last_default()(this.yearItems.map(i => i.year));
    }
